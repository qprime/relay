...
| comm buffer | Pending inter-PLC messages promoted each scan. A message becomes visible at the consumer's first scan top whose SimClock time is strictly later than the sending scan's, so delivery costs up to one *consumer* scan period — the phrasing survives independent per-PLC periods, since it names no global iteration. Plant routes are exempt: a sensor wired to the input terminals is sampled at scan top, not delivered over a network. The C++ host does not charge this cost, which makes the sim the conservative oracle (today's strategy is tag-based; address-based / Modbus TCP is planned) |
...
| send record | What a message carried, recorded where it was emitted: `(count, value)` in `ScanRecord.sends`. `count` is the per-sender, per-key cumulative send count carried on the message; sequence numbers are only meaningful within one sender's space, and matching them across senders is not attribution. Never derived from SimClock. `value` is what the send actually carried — a producer that sends every scan carries `False` long before the real event, so a count alone cannot locate an emission |
| assertion signal resolution | A comm tag resolves on its **producer**, from `sends`, anchored to the first *truthy* send. Every other name resolves `outputs` first then the I/O image, across every PLC. The split is what lets `PRECEDES` measure a send→act gap instead of reading both endpoints off one `ScanRecord` |
